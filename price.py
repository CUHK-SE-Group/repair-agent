...
# defects4j total
df_bugfix = pd.read_csv('result/defects4j/results_full-appendix_1shot_gpt-4_10try_temp=1.0_archived.csv', sep=',', encoding='utf-8')
bug_slug_ids = df_bugfix['slug'].tolist()
...
df_artifact = df_artifact[df_artifact['slug'].isin(bug_slug_ids)]
...
df_raw = df_raw[df_raw['slug'].isin(bug_slug_ids)]
...
# defects4j correct
df_bugfix = pd.read_csv('result/defects4j/results_full-appendix_1shot_gpt-4_10try_temp=1.0_archived.csv', sep=',', encoding='utf-8')
df_result = pd.read_csv('result/defects4j/evaluation_full-appendix_1shot_gpt-4_10try_temp=1.0_archived.csv', sep=',', encoding='utf-8')
fixed_bug_slug_ids = df_result[df_result['reward']==True]['slug'].tolist()
df_bugfix = df_bugfix[df_bugfix['slug'].isin(fixed_bug_slug_ids)]
df_bugfix['input_tokens'] = df_bugfix['bug'].apply(lambda x: count_tokens(x, 'gpt-4'))
input_token_mean = df_bugfix['input_tokens'].mean()
df_bugfix['output_tokens'] = df_bugfix['fix'].apply(lambda x: count_tokens(x, 'gpt-4'))
output_token_mean = df_bugfix['output_tokens'].mean()
...
input_mean = input_token_mean + test_method_token_mean + comment_token_mean + count_tokens(EXAMPLE_INPUT_FUNC_AGENT, 'gpt-4') + count_tokens(AGENT_PROMPT.replace('{BUGGY_COMMENT}','').replace('{ERROR_MESSAGE}','').replace('{FAILED_TEST}','').replace('{BUGGY_CODE}',''), 'gpt-4')
input_price = input_mean * input_per_price * sampling_number
print(f"input token mean for correct fixed bugs: {input_mean}, input context price mean: {input_price}")
output_price = output_token_mean * output_per_price * sampling_number
print(f"output token mean for correct fixed bugs: {output_token_mean}, output context price mean: {output_price}")
print(f"total bug-fix price mean for correct fixed bugs: {input_price + output_price}")
...
input_mean = input_token_mean + test_method_token_mean + comment_token_mean + count_tokens(EXAMPLE_INPUT_FUNC_AGENT, 'gpt-4') + count_tokens(AGENT_PROMPT.replace('{BUGGY_COMMENT}','').replace('{ERROR_MESSAGE}','').replace('{FAILED_TEST}','').replace('{BUGGY_CODE}',''), 'gpt-4')
input_price = input_mean * input_per_price * sampling_number
print(f"input token mean for incorrect fixed bugs: {input_mean}, input context price mean: {input_price}")
output_price = output_token_mean * output_per_price * sampling_number
print(f"output token mean for incorrect fixed bugs: {output_token_mean}, output context price mean: {output_price}")
print(f"total bug-fix price mean for incorrect fixed bugs: {input_price + output_price}")